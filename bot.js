...
function connectSocket() {
    console.log('Connexion au serveur PlaceQC...')

    socket = new WebSocket(`wss://${serverUrl}/api/ws`);

    socket.onopen = function () {
        console.log('Connecté au serveur PlaceQC!')
    ...
    socket.onmessage = async function (message) {
        ...
        switch (data.type.toLowerCase()) {
            case 'map':
                console.log(`Nouvelle carte chargée (raison: ${data.reason ? data.reason : 'connecté au serveur'})`)
                currentOrders = await getMapFromUrl(`${serverProtocol}://${serverUrl}/maps/${data.data}`);
                hasOrders = true;
                break;
            default:
                break;
    ...
    socket.onclose = function (e) {
        console.warn(`Le serveur PlaceQC s'est déconnecté : ${e.reason}`)
...
async function attemptPlace() {
    ...
    for (const i of order) {
        ...
        console.log(`Essai de publier un pixel sur ${x}, ${y}...`)
    ...
    console.log(`Tous les pixels sont déjà au bon endroit ! Réessayez dans 30secondes...`)
